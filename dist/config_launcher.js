...
async function setupHotReload(fs, config, configPath, serverName, setupAll) {
...
async function findConfigFile(fs, providedPath) {
...
const configPath = await findConfigFile(fs, args.config);
if (!configPath) {
...
if (args.setup !== undefined || args.all) {
    await setupHotReload(fs, config, configPath, args.setup, args.all);
    Deno.exit(0);
}

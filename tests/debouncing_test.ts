...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      console.log(
        `Restart indicators: ${restartCount}, Kill indicators: ${killCount}, Start indicators: ${startCount}`,
      );
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const startCount = (logs.match(/✅|Server started|started/g) || []).length;
      assertEquals(startCount >= 2, true, "Both restarts should complete successfully");

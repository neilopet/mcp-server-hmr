...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      assertEquals(logs.includes("📝") || logs.includes("File") || logs.includes("modify"), true,
        "Should detect file change");
        
      // 2. Restart initiated
      assertEquals(logs.includes("🔄") || logs.includes("restarting") || logs.includes("restart"), true,
        "Should initiate restart");
        
      // 3. Old server killed
      assertEquals(logs.includes("🛑") || logs.includes("Killing") || logs.includes("kill"), true,
      ...
      assertEquals(logs.includes("📢") || logs.includes("tools") || logs.includes("notification"), true,

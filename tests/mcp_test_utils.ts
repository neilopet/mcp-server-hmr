...
export function assertRestartSequence(logs: string): void {
  ...
  assertEquals(
    logs.includes("📝 File modify") || logs.includes("File change detected"),
    true,
  ...
  assertEquals(
    logs.includes("🔄") || logs.includes("restarting"),
    true,
  ...
  assertEquals(
    logs.includes("🛑") || logs.includes("Killing server"),
    true,
    "Should kill old server",
  );

  assertEquals(
    logs.includes("✅") || logs.includes("Server started"),
    true,

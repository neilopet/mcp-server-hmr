 ...
 */
function findLatestNodeVersion(): string | null {
  const home = homedir();
 ...
 */
function getMcpmonCommand(): { command: string; args: string[] } {
 ...
 */
function unwrapMcpmonConfig(serverConfig: MCPServerConfig): MCPServerConfig {
 ...
 */
function listServers(config: MCPServersConfig, configPath: string): void {
 ...
 */
function restoreConfig(configPath: string): void {
  // Find latest backup
  ...
  const backups = files
    .filter((f: string) => f.startsWith(backupPattern))
    .sort()
    .reverse();
 ...
 */
export function setupCommand(serverName: string | undefined, options: any): void {
  executeSetup(
    options.config,
    serverName || null,
    options.list || false,

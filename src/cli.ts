...
/**
 * Interface for process information from ps command
 */
 ...
 */
function parseProcessInfo(line: string): ProcessInfo | null {
 ...
 */
async function findMcpmonProcesses(): Promise<ProcessInfo[]> {
 ...
 */
async function findMcpmonDockerContainers(): Promise<string[]> {
 ...
 */
async function findOrphanedContainers(verbose: boolean = false): Promise<ContainerInfo[]> {
  return new Promise((resolve) => {
 ...
 */
async function inspectContainer(containerId: string, verbose: boolean = false): Promise<ContainerInfo | null> {
  return new Promise((resolve) => {
 ...
 */
async function cleanupOrphanedContainers(containers: ContainerInfo[], verbose: boolean = false): Promise<{ stopped: string[], failed: string[] }> {
 ...
 */
async function terminateProcess(pid: number, verbose: boolean = false): Promise<boolean> {
  try {
    if (verbose) {
 ...
 */
async function stopDockerContainers(containerIds: string[], verbose: boolean = false): Promise<{ stopped: string[], failed: string[] }> {
  const stopped: string[] = [];
  const failed: string[] = [];
 ...
 */
async function performCleanup(options: { force?: boolean, verbose?: boolean }): Promise<void> {
  ...
  let mcpmonProcesses: ProcessInfo[] = [];
  try {
  ...
  let orphanedContainers: ContainerInfo[] = [];
  try {

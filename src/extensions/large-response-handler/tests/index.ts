...
import largeResponseHandlerExtension, { LargeResponseHandlerExtension } from '../index.js';
...
export class LargeResponseHandlerTestSuite implements ExtensionTestSuite {
  ...
  readonly extension: any = largeResponseHandlerExtension;
  readonly metadata = {
   ...
   */
  private defineInitializationTests(getContext: () => MockExtensionContext): void {
   ...
   */
  private defineResponseDetectionTests(getContext: () => MockExtensionContext): void {
    describe('response detection', () => {
      ...
      it('should detect responses above size threshold', async () => {
        ...
        expect(result).toBeDefined();
        expect(getContext().logger.info).toHaveBeenCalledWith(
   ...
   */
  private defineStreamingTests(getContext: () => MockExtensionContext): void {
   ...
   */
  private defineToolInjectionTests(getContext: () => MockExtensionContext): void {
   ...
   */
  private defineToolCallTests(getContext: () => MockExtensionContext): void {
    describe('tool calls', () => {
      beforeEach(async () => {
        await this.extension.initialize(getContext());
      });
   ...
   */
  private defineProgressNotificationTests(getContext: () => MockExtensionContext): void {
    ...
    describe('integration scenarios', () => {
      ...
      it('should handle complete large response workflow', async () => {
        await this.testHarness.withExtension('large-response-handler', async () => {
          ...
          // Should receive progress notifications
          const notification = await this.testHarness.expectNotification(

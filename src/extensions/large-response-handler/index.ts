...
export default class LargeResponseHandlerExtension implements Extension {
  ...
  private async createDuckDBDatabase(dataFile: string, datasetId: string): Promise<DatabaseInfo> {
    ...
    try {
      ...
      this.context?.logger.info(
        `DuckDB database created successfully: ${tableName} (${rowCount} rows, ${columns.length} columns)`
      );
  ...
  private generateSampleQueries(tableName: string, columns: Array<{name: string, type: string}>, rowCount: number): string[] {
    ...
    const numericColumns = columns.filter(col => 
      col.type.includes('INT') || col.type.includes('DOUBLE') || col.type.includes('FLOAT')
    );
    ...
    if (rowCount > 10 && rowCount < 1000000) {
      const categoricalColumns = columns.filter(col => 
        col.type.includes('VARCHAR') || col.type.includes('STRING')
      );
  ...
  private async processLargeResponse(message: any, response: any): Promise<any> {
    try {
      ...
      this.context?.logger.info(
        `Large response persisted: ${responseSize} bytes → ${datasetId} (${paths.dataFile})`
      );
      ...
      if (filter) {
        filteredDatasets = datasets.filter(dataset => {
          ...
          if (filter.date_from || filter.date_to) {
            ...
            if (fromDate && datasetDate < fromDate) return false;
            if (toDate && datasetDate > toDate) return false;
          }
          
          // Tool filtering
          if (filter.tool && dataset.tool !== filter.tool) return false;
  ...
  private async analyzeWithDuckDB(datasetId: string | undefined, query: string): Promise<any> {
    try {
      ...
      let dataset: {id: string, timestamp: number, path: string} | null;
      if (datasetId) {

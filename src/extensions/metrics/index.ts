...
import type { Extension, ExtensionContext, MessageHook, ToolDefinition } from '../interfaces.js';
...
export default class MetricsExtension implements Extension {
  ...
  private async beforeRestart(): Promise<void> {
  ...
  private async handleToolCall(toolName: string, args: any): Promise<any | null> {
    if (toolName !== 'mcpmon.get-metrics') return null;
  ...
  private formatPrometheus(): any {
    ...
    for (const [method, count] of Object.entries(this.metrics.requests.byMethod)) {
      lines.push(
        `# HELP mcpmon_requests_by_method{method="${method}"} Requests by method`,
        `# TYPE mcpmon_requests_by_method counter`,
        `mcpmon_requests_by_method{method="${method}"} ${count}`
      );
    }
  ...
  private stopUptimeUpdater(): void {

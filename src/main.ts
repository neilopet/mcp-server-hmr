 ...
 * 
 * A development tool that provides hot-reloading for MCP servers.
...
const logLevel = Deno.env.get("MCP_LOG_LEVEL") || "info";
const restartDelay = parseInt(Deno.env.get("MCP_RESTART_DELAY") || "300");
...
const commandArgs = serverArgs.split(" ").filter(arg => arg.trim() !== "");

// Determine entry file to watch (explicit override or infer from args)
let entryFile: string | null = watchFile || null;
...
console.error(`🚀 Starting MCP Hot-Reload Proxy`);
